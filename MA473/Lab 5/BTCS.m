function [x1, x, t, u_numerical, u_surface, u_num2] = BTCS(T, K, r, sigma,delta, S_min, S_max, method)
    h = 1;
    k = h^2/70;
    ...
    for i=time-1:-1:1
        ...
        for j =2:steps-1
            t1 = (sigma*(j-1)*h)^2;
            t2 = (r - delta)*(j-1)*h;
            M(j,j-1) = (-0.5*t1*k)/h^2 + (t2*k*0.5)/h;
            M(j,j+1) = (-0.5*t1*k)/h^2 - (t2*k*0.5)/h;
            M(j,j) = 1 + r*k + (t1*k)/h^2;
            Y(j) = U(i+1, j);
        end
